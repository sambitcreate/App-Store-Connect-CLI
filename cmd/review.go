...
Examples:
  ...
  asc review items-add --submission "SUBMISSION_ID" --item-type appStoreVersions --item-id "VERSION_ID"`,
		...
		Subcommands: []*ffcli.Command{
			ReviewDetailsGetNounCommand(),
			ReviewDetailsForVersionNounCommand(),
			ReviewDetailsCreateNounCommand(),
			ReviewDetailsUpdateNounCommand(),
			ReviewAttachmentsListCommand(),
			ReviewAttachmentsGetCommand(),
			ReviewAttachmentsUploadCommand(),
			ReviewAttachmentsDeleteCommand(),
			ReviewSubmissionsListNounCommand(),
			ReviewSubmissionsGetNounCommand(),
			ReviewSubmissionsCreateNounCommand(),
			ReviewSubmissionsSubmitNounCommand(),
			ReviewItemsListNounCommand(),
			ReviewItemsAddNounCommand(),
			ReviewItemsRemoveNounCommand(),
		},
